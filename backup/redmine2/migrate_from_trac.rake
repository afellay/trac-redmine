...
/
    ...
    print "Migrando tickets: "
		#TracTicket.find_each(:batch_size => 200, :conditions => {:id => 248}) do |ticket|
   ...
   #   puts
       ...
       # if change.field == 'GDS' &&
			 #		n.details << JournalDetail.new(
				#		:property => 'cf',
				#		:prop_key => custom_field_map['GDS'].id.gsub(/[|]/,''),
				#		:old_value => change.oldvalue,
				#		:value => change.newvalue
				#		)
				#end
      ...
      ticket.attachments.each do |attachment|
		...
		# Custom fields
          ...
          if custom.name == 'gds_solicitudes' && !custom.value.blank?
            
            h[custom_field.id] = custom.value.gsub('[http://','').gsub(']','')
            #h[custom_field.id] = h[custom_field.id]
          else
      ...
      if custom_field_map['Ticket Migrado'] 
		 		custom_values[custom_field_map['Ticket Migrado'].id] = "repositorio.siu.edu.ar/trac/pilaga/ticket/#{ticket.id}"
		 	end
    ...
    #def self.encode(text)
      #  text.to_s.force_encoding(@charset).encode('UTF-8')
		...
		prompt('Ruta de Trac', :default => '/var/trac/pilaga') {|directory| TracMigrate.set_trac_directory directory.strip}
		...
		unless %w(sqlite sqlite3).include?(TracMigrate.trac_adapter)
			...
			prompt(' - base', :default => 'trac_pilaga') {|name| TracMigrate.set_trac_db_name name}
		...
		prompt(' - identificador del proyecto', :default => 'pilaga') {|identifier| TracMigrate.target_project_identifier identifier}
		puts
		puts
